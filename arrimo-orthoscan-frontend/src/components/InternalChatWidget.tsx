...
import { listInternalChatMessages, sendInternalChatMessage, type InternalChatMessage } from '../repo/internalChatRepo'
...
export default function InternalChatWidget() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void listInternalChatMessages().then((result) => {
      if (!isMounted) return
      if (!result.ok) {
  ...
  }, [currentUser, displayName, isSupabaseMode])
  ...
  const handleSend = async () => {
    ...
    setError(null)
    const result = await sendInternalChatMessage({ senderUserId: currentUser.id, senderName: displayName, body })
    if (!result.ok) {
      setError(result.error)
      return
    }
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed bottom-20 right-5 z-40 w-[360px] max-w-[calc(100vw-2rem)] rounded-xl border border-slate-200 bg-white shadow-2xl">
